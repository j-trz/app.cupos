...
// parseExtraEmails separa la lista de casillas adicionales de una
// NotificationTemplate (coma, punto y coma o salto de línea), recortando
...
// ResolveNotificationText resuelve el título/mensaje/icono/color/prioridad/
// casillas-extra a usar para una notificación de tipo `code`: si un admin
// personalizó una NotificationTemplate (global o de la agencia dada), la usa
// (con los placeholders {{clave}} de data reemplazados); si no, cae a
...
// falta de seed. icon/color/priority/extraEmails quedan vacíos si no hay
// plantilla personalizada, y createNotification les aplica sus defaults de
// siempre.
func ResolveNotificationText(agencyCode, code, defaultTitle, defaultMessage string, data map[string]string) (title, message, icon, color, priority string, extraEmails []string) {
	if tpl, ok := resolveNotificationTemplate(agencyCode, code); ok {
		return renderPlaceholders(tpl.Title, data), renderPlaceholders(tpl.Message, data), tpl.Icon, tpl.Color, tpl.Priority, parseExtraEmails(tpl.ExtraEmails)
	}
	return defaultTitle, defaultMessage, "", "", "", nil
}
...
func NotifyUserByCode(userID uuid.UUID, createdBy *uuid.UUID, agencyCode, code, defaultTitle, defaultMessage string, data map[string]string) {
	title, message, icon, color, priority, _ := ResolveNotificationText(agencyCode, code, defaultTitle, defaultMessage, data)
	createNotification(models.Notification{
...
// Es la única *ByCode que además dispara las "casillas de correo adicionales"
// configuradas en la plantilla (si las tiene): un mismo evento de negocio
// siempre dispara exactamente una llamada a NotifyRoleByCode("admin", ...),
// a diferencia de NotifyUserByCode/NotifyAgencyByCode que pueden coexistir
// con ella para el mismo evento — mandarlas ahí también duplicaría el email.
func NotifyRoleByCode(role string, createdBy *uuid.UUID, code, defaultTitle, defaultMessage string, data map[string]string) {
	title, message, icon, color, priority, extraEmails := ResolveNotificationText("", code, defaultTitle, defaultMessage, data)
	createNotification(models.Notification{
		Type: code, Title: title, Message: message, Icon: icon, Color: color, Priority: priority,
		TargetRole: role, CreatedBy: createdBy,
	})
	for _, addr := range extraEmails {
		if err := SendRawEmail("", addr, title, message); err != nil {
			LogFailure("email", fmt.Sprintf("No se pudo enviar el aviso a la casilla adicional %s", addr), err.Error())
		}
	}
}
...
func NotifyBroadcastByCode(createdBy *uuid.UUID, code, defaultTitle, defaultMessage string, data map[string]string) {
	title, message, icon, color, priority, _ := ResolveNotificationText("", code, defaultTitle, defaultMessage, data)
	createNotification(models.Notification{
		Type: code, Title: title, Message: message, Icon: icon, Color: color, Priority: priority,
		CreatedBy: createdBy,
	})
