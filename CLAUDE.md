...
Hay una base de conocimiento persistente del proyecto en el vault de Obsidian **`app-cupos`** (`C:\Users\julian.estefan\Desktop\app-cupos\`, se interactúa vía el CLI `obsidian`): arquitectura, catálogo de API, modelo de datos, flujos de negocio con diagramas, historial de bugs resueltos y el backlog de feedback de testing. Consultarlo antes de re-derivar algo de cero — especialmente `006 - Operación y Mantenimiento` (gotchas + bugs ya cerrados) y `005 - Arquitectura y Datos` (modelo de datos).
