...
- **Herramientas de IA del ecosistema (global, ver regla 25 de `Gotchas y Reglas de Oro.md`)**: `codebase-memory-mcp` (indexar el repo antes de explorar código a mano con grep) e `impeccable` (skill de diseño — ampliado 2026-08-15 a pedido explícito de Julian: ahora gobierna **todo** el diseño del proyecto, incluidas las pantallas `Gestion*.jsx` internas, pero solo para **elevar** lo que ya existe — tablas, iconografía, jerarquía visual — nunca para reemplazarlo por la estética genérica de impeccable ni por shadcn/otro kit; sigue usando el kit propio y la paleta slate ya establecida, la Guía de Diseño es la base que impeccable eleva, no reemplaza). `superpowers` está desactivado globalmente. Detalle/por qué de cada decisión en `~/.claude/rules/ai-tooling-strategy.md` (fuera del repo).
