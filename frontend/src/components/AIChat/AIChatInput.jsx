    ...
    return (
        <div className="border-t border-zinc-200 bg-white p-3">
            ...
            {attachments.length > 0 && (
                <div className="mb-2 flex flex-wrap gap-1.5 bg-zinc-50 rounded-xl p-2 max-h-24 overflow-y-auto">
                    {attachments.map((img) => (
                        <div
                            key={img.id}
                            className="relative flex items-center gap-1.5 bg-white border border-zinc-200 rounded-lg p-1 pr-1.5 max-w-[140px] shadow-sm shrink-0"
                        >
                            <img
                                src={img.dataUrl}
                                alt="preview"
                                className="w-8 h-8 object-cover rounded border border-zinc-100 shrink-0"
                            />
                            <p className="text-[10px] font-medium text-zinc-700 truncate flex-1 leading-tight select-none">
                                {img.name}
                            </p>
                            <button
                                type="button"
                                onClick={() => setAttachments((prev) => prev.filter((a) => a.id !== img.id))}
                                className="p-0.5 hover:bg-zinc-100 rounded text-zinc-400 hover:text-red-500 transition-colors shrink-0"
                                title="Eliminar"
                            >
                                <X className="w-3 h-3" />
                            </button>
            ...
            <div className="flex items-end gap-2">
                ...
                <div className="flex-1 relative">
                    <textarea
                        ...
                        disabled={isLoading}
                        rows={1}
                        className="w-full resize-none bg-zinc-100 border border-transparent focus:border-zinc-300 focus:bg-white rounded-xl px-3 py-2 text-sm text-zinc-900 placeholder-zinc-400 outline-none transition-colors disabled:opacity-50 overflow-y-auto"
                ...
                <button
                    ...
                    className="p-2 bg-zinc-900 hover:bg-zinc-700 disabled:bg-zinc-200 text-white disabled:text-zinc-400 rounded-lg transition-all duration-150 disabled:cursor-not-allowed hover:scale-105 active:scale-95"
                    title="Enviar"
                >
                    {isLoading ? (
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                    {[
                        ...
                        <button
                            ...
                            className="px-2.5 py-1 text-[11px] bg-zinc-100 hover:bg-zinc-200 text-zinc-600 rounded-full border border-zinc-200 transition-colors"
                        >
                            {s.label}
                        </button>
                    ))}
