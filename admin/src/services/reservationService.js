...
class ReservationService {
  ...
  static async _fetchAvailability() {
    try {
      // Obtener datos específicos de productos/disponibilidad
      ...
      // Log para debuggear qué datos están llegando
      console.log(
        "Datos completos para disponibilidad (productos):",
        result.data.slice(0, 5)
      );

      // CORREGIDO: Con la estructura real de productos que me mostraste
      const availabilityData = result.data.filter((item) => {
        // Verificar que tenga la estructura de productos real
        const hasValidProductFields =
          item.codigo_cupo &&
          item.destino &&
          item.compania &&
        ...
        if (hasValidProductFields) {
          console.log("✅ Producto válido identificado:", {
            codigo: item.codigo_cupo,
          ...
          console.log("❌ Producto inválido:", item);
        }

        return hasValidProductFields;
      });

      console.log(
        `✅ Filtrados ${availabilityData.length} productos válidos de ${result.data.length} totales`
      );
