...
**Resuelto 2026-08-26** (a pedido puntual de Julian, de la lista "Alto"/"Medio"): reportes legacy (`/api/reports/*`) y `/transfers/all` ya no filtran cross-agencia (scoping en `loadDataFromDB()` y `ListTransfers`); templates de email/notificación ahora exigen permiso + ownership (`callerOwnsTemplateAgency`, con backfill idempotente de permisos para `AGENCY_ADMIN`); rate limiting agregado (global 300 req/min por IP + 15 req/5min en `/auth/login`); credencial de Netviax Atlas (`AtlasConfig.Clave`) cifrada en reposo (AES-256-GCM, autocura valores legacy en texto plano); `.github/dependabot.yml` agregado para actualizaciones automáticas de dependencias (Go, npm, GitHub Actions) — mitiga el punto de "sin verificación de CVEs de forma continua", aunque **falta un paso manual de Julian**: activar "Dependabot alerts"/"Security updates" en Settings → Security del repo en GitHub, el archivo por sí solo no lo prende. Detalle completo en [[Historial de Bugs Resueltos]].

**Sigue sin resolver**: `agency_admin` confundido con `admin` en varios chequeos de ownership (tool de IA `mis_reservas`, `GetReservationByID`, `RequestCancellation`, `RequestGroupCancellation`, `group_handler.go`), CVEs específicas ya identificadas en dependencias actuales (`golang-jwt`, `pgx`, `bytedance/sonic` — Dependabot las va a ir actualizando, pero no fue una actualización manual dirigida todavía), exposición de `EmailSMTPConfig.SMTPPass`/`AIProvider.APIKey` en texto plano, `GetSystemStatus` filtrando PII cross-agencia, `RestoreBackupHandler` sin validar esquema, la crítica de seguridad frontend (`xlsx`), la race condition de sobreventa en `CreateReservation` (QA backend), y el resto de QA frontend — ver el detalle completo en el reporte.
