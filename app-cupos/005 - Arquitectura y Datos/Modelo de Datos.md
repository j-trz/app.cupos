...
- **`Destino`** (agregado 2026-08-15): copiado de `Reservation.VueloDestino` al emitir — un valor limpio (ej. "RIO"), no algo derivado de `Ruta`.
- **`Segmentos`** (agregado 2026-08-15, `datatypes.JSON`, mismo patrón que `WhiteLabelConfig.Config`/`SystemSetting.Value`): `Ruta` (texto libre, ej. `"1JA763 31DEC MVDGIG 1432 1715 2JA762 09JAN GIGMVD 0950 1200"`) parseado tramo por tramo — **un ticket puede tener más de un tramo** (ida/vuelta, escalas) — en campos propios: `compania`, `vuelo`, `origen`, `destino`, `fecha_salida`, `salida`, `llegada`, `fecha_llegada`, `next_day`. Se calcula una sola vez al emitir con `services.ParseRuta()` (`backend-go/pkg/services/itinerary_parser.go`), un **puerto 1:1 a Go** de `parseRuta()`/`ItineraryTable.jsx` (frontend) — mismo algoritmo, mismas 3 estrategias de split por regex, para que backend y frontend nunca diverjan en cómo leen el mismo texto. Pensado explícitamente para alimentar al backoffice/Atlas con datos normalizados, no texto libre a re-parsear (ver `007 - Integración Netviax Atlas/Envío de Tickets Emitidos (Pendiente).md`). El modal de detalle (`BandejaTickets.jsx`) muestra el Origen del primer tramo y la lista completa de tramos desde este campo (con `parseRuta()` del lado del frontend como fallback solo para tickets emitidos antes de que existiera esta columna). Los tickets emitidos antes de este fix quedan con `destino`/`segmentos` vacíos y el `PNR` viejo (no hay backfill automático posible, ver limitación de abajo) — se autocuran la próxima vez que se les toca el número de ticket.
...
- **E-ticket completo — campos de backoffice snapshoteados** (agregado 2026-08-15, a pedido explícito para que la tabla de `BandejaTickets.jsx` muestre cada dato "por separado... para que no falle el dato"): `TipoPasajero`, `TipoDocumento` (`"CI"` o `"Pasaporte"`, según cuál de `Passenger.Documento`/`Pasaporte` estaba cargado — vacío en el fallback legado a nivel reserva, que no distingue los dos), `PedidoID` (`Reservation.PedidoID`), `Vendedor` (email de `Reservation.CreatedBy`, resuelto una vez por `resolveVendedorEmail()`), `FechaReserva` (`Reservation.CreatedAt`), y la franquicia de equipaje (`CarryOn`/`HandBag`/`CheckedBag` + `*Kg`, mismo shape que `Product` — pensado para reusar `BaggageFranchise.jsx` sin mapeo). Todos son snapshot al emitir, mismo patrón que `Agencia`/`Ficha`/`Compania` (ver limitación de no-JOIN abajo). La tabla de `BandejaTickets.jsx` ahora explota **una fila por tramo** (no por ticket) para poder mostrar `N° Segmento`/`Compañía`/`Vuelo`/`Fecha Salida`/`Origen`/`Destino`/`Hora Salida`/`Hora Llegada` como columnas propias de cada tramo — los campos a nivel ticket (PNR, pasajero, tarifa, etc.) se repiten en cada fila de tramo del mismo ticket.
