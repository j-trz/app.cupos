...
- **`graphify`** (instalado 2026-08-27, scopeado a este repo con `graphify install --project`, no global): grafo de conocimiento del **código** (`backend-go`+`frontend`, 230 archivos → 1605 nodos/4414 edges/117 comunidades), extraído 100% local vía AST (`tree-sitter`, sin LLM, sin costo). Antes de grepear/leer archivos sueltos para preguntas estructurales, usar `graphify query "<pregunta>"` / `graphify path "A" "B"` / `graphify explain "X"` — hay un hook `PreToolUse` (nudge, no bloqueante) que ya lo recuerda en cada `Read`/`Grep`. Rebuild automático post-commit/post-checkout (solo AST, sin costo). **El vault `app-cupos/` (este mismo vault) NO está fusionado al grafo** — su extracción semántica requiere un backend LLM (`ANTHROPIC_API_KEY`/`OPENAI_API_KEY`/etc. u Ollama local) que no estaba disponible al instalar; ninguno de los 3 intentos (`claude-cli`, `claude` con `ANTHROPIC_BASE_URL` de la sesión, backend sin key) funcionó, así que sigue siendo dos fuentes separadas (grafo de código vs. vault a mano) en vez del cerebro único que documenta el plan original de instalación. `graphify-out/` está en `.claudeignore` (no se lee crudo, cuenta como grafo) y `graphify-out/cost.json` en `.gitignore` (local, no se versiona).
