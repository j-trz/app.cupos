...
4. `BandejaTickets.jsx`: la tabla muestra las 26 columnas pedidas en el orden que Julian especificó, con los datos de ruta por tramo (nro. de segmento, compañía, vuelo, fechas, origen/destino, horas) separados por campo. El modal de detalle también se amplió con los mismos campos nuevos.
...
**Corrección inmediata (misma sesión)**: la primera implementación del punto 4 explotaba la tabla en **una fila por tramo** (`buildTicketRows()`) — Julian marcó que esto es incorrecto: "1 reserva puede tener multiples tramo bajo el mismo pnr y tkt", una fila por tramo sugiere visualmente boletos separados cuando en realidad comparten el mismo PNR y número de ticket. Corregido a **una fila por ticket**: los valores de ruta se apilan como múltiples líneas dentro de la misma celda de cada columna (componente `SegmentStack`), alineadas entre columnas por índice de tramo — sigue separando cada dato, pero sin ambigüedad de que es un solo ticket. Las acciones (Void/Sync), que con la explosión por fila aparecían duplicadas por tramo, vuelven a aparecer una sola vez por ticket.
