...
func isTienda(agencia string) bool {
	agNorm := normalize(agencia)
	tiendaNames := []string{"tienda viajes", "tienda de viajes", "tienda de viajes srl", "tienda", "tienda viajes srl"}
	for _, name := range tiendaNames {
		if agNorm == name {
			return true
		}
	}
...
func DestinosCompaniaHandler(c *gin.Context) {
	...
	for _, cupo := range cuposUnicos {
		...
		// Keep only short codes if desired, but let's be flexible and accept whatever is returned
